...
var OriginalSequencePairOrder = SequencePairOrder;
...
//For keeping track of whether to show feedback or if they experiment is ending
var experimentWillEnd = false;

//Initial feedback duration (will set to 1 when experiment is ending)
...
var getFeedback = function () {
	...
	} else if (experimentWillEnd == true) {
		console.log("Feedback set to null because the experiment is ending.");
		return [''];
    } else {
		//return ['<div class = centerbox><div class = center-text> Click "Next" to continue. </div></div>'];
		return ['<div class = centerbox><div class = center-text> <p>La pr&oacute;xima secuencia de d&iacute;gitos comenzar&aacute; en breve.</p> </div></div>'];
    }
}
...
var block_start_page = {
    type: 'instructions',
    pages: [
        '<div class = centerbox><p class = block-text>Respire profundo y haga clic en el bot&oacute;n Continuar para comenzar la prueba de memoria.</p></div>'
    ],
    allow_keys: false,
	...
	page_number_label: 'P&aacute;gina',
    data: {
...
function generate_backward_block_fixed(digit_sequence) {
    ...
    for (var ii = 0; ii < digit_sequence.length; ii++) {
        ...
        var trial_start_page = {
            ...
            is_html: true,
            stimulus: function () {
                return '<div class = centerbox><div class = center-text>' + digit_sequence[corr_history.length] + ' d&iacute;gitos</p></div>';
            },
            data: {
...
//         alert("Current sequence pair order: " + JSON.stringify(SequencePairOrder));
		...
		var oneDigitFromParticipant = {
			type: 'html-keyboard-response',
			stimulus: '<p>Por favor, escriba cada d&iacute;gito en el orden inverso al que apareci&oacute;.</p>',
            data: {
...
// 			prompt: '<p>Just type the numbers themselves, one by one. <BR> So far you have typed the following: </p>' + JSON.stringify(fullResponse),
			prompt: function(){
			  return "<p>Tan solo teclee los n&uacute;meros, uno por uno. <BR><BR> Hasta ahora ha escrito lo siguiente: </p>" + fullResponse.toString().replace(/,/g, ', ');
			},
		...
		var AfterFinalDigitFromParticipant = {
			...
			on_finish: function (data) {			
				...
				console.log('Checking the current value in SequencePairOrder array: ', SequencePairOrder[currentNumberDigits-2]);
				if ((SequencePairOrder[currentNumberDigits-2] == 4) || (SequencePairOrder[currentNumberDigits-2] == 5) || (currentDigit == 2)) { 
					currentIncorrect = 0;
				}
				...
				if (correct) {
					feedback = '<span style="color:green">&iexcl;Correcto!</span>';
					if (max_level < digit_sequence[corr_history.length]) {
						max_level = digit_sequence[corr_history.length];
					}
				} else {
					feedback = '<span style="color:red">Incorrecto.</span>';
				...
				//If you have two (or more) wrong and you've done both trials in a set, then set the feedback to null so that it doesn't sound like there's more.
				if ((currentIncorrect > 1) && (SequencePairOrder[currentNumberDigits-2] >= 7)) {
					experimentWillEnd = true;
					feedbackDuration = 1;
				}
		...
		// feedback, version which removes 'next' button always
		var feedback_page_html = {
			type: 'html-keyboard-response',
            ...
            data: {
                exp_stage: "feedback_page_html_" + ii.toString()
            },
			on_finish: function (data) {
				//If they get two wrong in the set of sequences for the current number of digits, end the memory task
				if ((currentIncorrect > 1) && (SequencePairOrder[currentNumberDigits-2] >= 7)) { //If you have two (or more) wrong and you've done both trials in a set, then end the experiment.
					jsPsych.endExperiment(' '); // message was "The experiment was ended due to a wrong answer"
					console.log('Ending experiment due to a wrong answer.');
				}
			}
        };
        block_sequence.push(feedback_page_html);
       ...
       // block_sequence.push(feedback_page); //Not included because we don't want the button for continuing.
    }
