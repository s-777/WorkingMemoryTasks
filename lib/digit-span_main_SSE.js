...
var getFeedback = function () {
	if (currentNumberDigits == 2) {
		...
		return ['<div class = centerbox><div class = center-text> The next sequence of digits will display shortly. </div></div>'];
    }
}
...
// 			prompt: '<p>Just type the numbers themselves, one by one. <BR> So far you have typed the following: </p>' + JSON.stringify(fullResponse),
		...
		// feedback, version which removes 'next' button always (not after first two digits by converting to HTML for those)
